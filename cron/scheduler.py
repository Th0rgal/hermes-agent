...
def _deliver_result(
    ...
    if (
        ...
        if queued_error:
            delivery_errors.append(queued_error)
        return "; ".join(delivery_errors) if delivery_errors else None
    ...
    if not _sanitized and not (content or "").strip():
        # Blank payload: fall through to the fail-closed empty-payload
        # handling below (nothing to sanitize, nothing to strip).
        pass
    elif not _sanitized:
        logger.info(
    ...
    for lane_value in lane_values:
        for part in lane_value.split(","):
            ...
            if not part or part.lower() in {"local", "origin", "all"}:
                continue
            # `project:<id|slug>` is a fork routing scheme, NOT a messaging
            # platform: it resolves to a live SessionDB session via
            # _resolve_project_route_target, so it is exempt from the
            # known-platform credential check like local/origin/all. Without
            # this every controller delivering `project:X` fails
            # blocked_config. [fork-delta]
