...
class DeliveryRouter:
    ...
    async def deliver_events(
        ...
        Semantics layered on top of :meth:`deliver`:

        * **Dedupe** — duplicate events (same :func:`dedupe_key`) within the
          batcher window collapse to a single rendering. Callers that want
          cross-call dedupe pass a shared ``batcher``; otherwise a fresh
          per-call batcher still dedupes within the call.
        * **Batching** — accepted events render as one digest message per
          flush (at most ``batcher.max_batch`` per digest) instead of one
          platform send per event.
        * **Structured silence** — events with ``silent=True`` NEVER reach a
          platform adapter. They are still written through the local delivery
          path so the local-origin audit trail is preserved (mirrors the
          "local delivery is never filtered" invariant above).

        Event payloads carry only local identifiers (job/project/session ids)
        by construction, so the digest and audit metadata leak no platform
        routing information.
        """
