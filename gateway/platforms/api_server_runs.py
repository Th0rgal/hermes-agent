            ...
            def _run_sync():
                ...
                with self._profile_scope(request_profile):
                    try:
                        ...
                        # /v1/runs does not go through GatewayRunner's
                        # compression_exhausted auto-reset. Without a child
                        # session the bound project stays on the bloated
                        # transcript and every later reply reprints "Cannot
                        # compress further". Fork once, then retry the same
                        # user turn on the empty child. [fork-delta]
                        if (
                            isinstance(r, dict)
