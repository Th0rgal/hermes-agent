...
class TestFetchOpenRouterModels:
    def test_live_fetch_recomputes_free_tags(self, monkeypatch):
        ...
        # Pin the curated picker list too: fetch_openrouter_models returns the
        # intersection of the curated list with the live catalog, so a test that
        # only mocks the live catalog silently depends on whatever the deployed
        # curated list currently contains (it drifts, and drops these ids).
        curated = [
    ...
    def test_permissive_when_supported_parameters_missing(self, monkeypatch):
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        curated = [("anthropic/claude-opus-4.8", ""), ("qwen/qwen3.7-max", "")]
        with patch("hermes_cli.model_catalog.get_curated_openrouter_models", return_value=curated), \
             patch("hermes_cli.models._urlopen_model_catalog_request", return_value=_Resp()):
            models = fetch_openrouter_models(force_refresh=True)
