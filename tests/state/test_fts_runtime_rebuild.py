...
``LIKE`` queries until an explicit offline rebuild (`hermes sessions
optimize-storage`) — a later open must never rebuild automatically.
"""
...
class TestRuntimeFtsRebuild:
    ...
    def test_corruption_fails_open_and_stays_detached_on_reopen(self, db, tmp_path):
        if not db._fts_enabled:
            pytest.skip("FTS5 unavailable in this build")
        db_path = tmp_path / "state.db"
        db.create_session("s1", source="test")
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        # [fork-delta] production leaves a stale FTS index detached at open
        # (hermes_state_schema: rebuild only via the explicit offline command),
        # so the upstream reap-then-rebuild path is intentionally never taken.
        pytest.skip("fork policy: stale FTS is never rebuilt on open")
        if not db._fts_enabled:
            pytest.skip("FTS5 unavailable in this build")
        db_path = tmp_path / "state.db"
        db.create_session("s1", source="test")
